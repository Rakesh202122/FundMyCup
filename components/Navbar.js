...
const Navbar = () => {
  ...
  return (
    <nav className='bg-gray-500 text-white flex justify-between items-center px-4 md:h-16 flex-col md:flex-row'>
      {/* <div className='logo font-bold text-lg justify-center items-center'><span><img src="/logo.png" alt="" width={130} /></span></div> */}
      <a href="/" className="flex items-center mb-4 sm:mb-0 space-x-3 rtl:space-x-reverse">
                <img src="/logo.png" className="h-16 md:h-14" alt="FundMyCup Logo" />
      </a>
...
<div id="dropdown" className={`z-10 ${showdropdown?"":"hidden"} absolute left-[125px] bg-white divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-700`}>
    <ul className="py-2 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownDefaultButton">
     ...
     {session && <button type="button" className="text-white bg-gradient-to-r from-cyan-500 to-blue-500 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-cyan-300 dark:focus:ring-cyan-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2" onClick={()=>{signOut()}}>Logout</button>}
